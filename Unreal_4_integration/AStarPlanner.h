...
//typedefs
typedef TArray<ENUM_ACTION_TYPE> ACTION_TYPE_LIST;
...
public:
	...
	// Initialisation.
	void			InitAStarGoalPlanner(AGOAPController* pAgent, AStarMapPlanner* pAStarMapPlanner, GoalAbstract* pGoal);
...
public:
	...
	virtual int32				GetNumAStarNeighbours(AGOAPController* pAgent, AStarNodeAbstract* pAStarNode);
	virtual ENUM_ASTAR_NODE_ID	GetAStarNeighbour(AGOAPController* pAgent, AStarNodeAbstract* pAStarNode, int32 iNeighbour, AStarStorageAbstract* pAStarStorage);

	virtual void		SetAStarFlags(ENUM_ASTAR_NODE_ID eAStarNode, uint64 dwFlags, uint64 dwMask);
	virtual uint64		GetAStarFlags(ENUM_ASTAR_NODE_ID eAStarNode);

	// Initialisation.
	void	BuildEffectActionsTable();
	void	InitAStarMapPlanner(AGOAPController* pAgent);
