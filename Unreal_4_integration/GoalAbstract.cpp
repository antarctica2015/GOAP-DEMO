...
//initialses the goal
void GoalAbstract::InitGoal(AGOAPController* pAgent, ENUM_GOAL_TYPE eGoalType)
{
	m_pAgent = pAgent;
...
{
	m_fActivationTime = GetCurrentTime();
...
void GoalAbstract::DeactivateGoal()
{
	if (m_fFrequency > 0.0f) {
		m_fNextRecalcTime = GetCurrentTime();
	}
}
...
{
	if (m_fRecalcRate > 0.0f) {
		m_fNextRecalcTime = GetCurrentTime() + m_fRecalcRate;
	}
}
