...
void AStarMachine::InitAStarMachine(AGOAPController* pAgent, AStarStorageAbstract* pAStarStorage, AStarGoalAbstract* pAStarGoal, AStarMapAbstract* pAStarMap)
{
	m_pAgent = pAgent;
	...
	while (true) {
		...
		for (iNeighbour = 0; iNeighbour < cNeighbours; ++iNeighbour) {
			...
			if (dwFlags & AStarMapAbstract::kASTAR_Open) {
				...
				pAStarNodeNeighbour->DebugPrintNeighbour();
			...
			// If our current best path is through this neighbour, we do not need to re-assess the neighbour.
			...
			// to this neighbour node from the current node than from its previous parent.
			...
			// If the node was previously closed, remove it from the Closed list, because it will be re-opened to re-explore its neighbours.
