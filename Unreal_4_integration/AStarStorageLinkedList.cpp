...
void AStarStorageLinkedList::AddToOpenList(AStarNodeAbstract* pAStarNode, AStarMapAbstract* pAStarMap)
{
	...
	uint64 dwFlags = pAStarMap->GetAStarFlags(pAStarNode->eAStarNodeID);
...
{
	// The AStar algorithm will never try to insert an already closed node in the Closed list, so no need to check for it.
...
{
	// No need to clear AStar flags, because when node is inserted into the Closed list, flags be be cleared and reset.
...
{
	// No need to clear AStar flags, because when node is inserted into the Open list, flags be be cleared and reset.
