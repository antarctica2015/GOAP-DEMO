...
//extern AAgentController* m_pAgent;
...
void AStarGoalPlanner::InitAStarGoalPlanner(AGOAPController* pAgent, AStarMapPlanner* pAStarMapPlanner, GoalAbstract* pGoal)
{
	m_pAgent = pAgent;
...
bool AStarGoalPlanner::IsPlanValid(AStarNodePlanner* pAStarNode)
{
	...
	while (pAction) {
		...
		if ((pAction->GetActionProbability(m_pAgent) < 1.0f) && (pAction->GetActionProbability(m_pAgent) < GetRandom(0.0f, 1.0f))) {
	...
	// The final node is the Goal return true if the goal is satisfied by the world state created by the sequence of actions.
...
{
	// Create a node with a specified AStarNodeID. The AIAStarMapPlanner converts between NodeIDs and Action enums.
...
void AStarMapPlanner::BuildEffectActionsTable()
{
	...
	int32 cActions = m_pAgent->GetActionStorage()->GetNumActions();

	for (int32 iAction = 0; iAction < cActions; ++iAction) {
		// Access an action.
		...
		for (iKey = 0; iKey < kWSKey_Count; ++iKey) {
			...
			if (pFlags->test(iKey)) {
				m_listEffectActions[iKey].Emplace(pAction->GetActionType());
			}
		}
	}
...
void AStarMapPlanner::InitAStarMapPlanner(AGOAPController* pAgent)
{
	if (m_pAgent == pAgent) {
		return;
	}
...
int32 CompareActions(const void *arg1, const void *arg2)
{
	if (!(arg1 && arg2))
	{
		return 0;
	...
	ActionAbstract* pAction1 = m_pAgent->GetActionStorage()->GetAction(eAction1);
	ActionAbstract* pAction2 = m_pAgent->GetActionStorage()->GetAction(eAction2);
...
//Return number of Neighbours for an AStarNode. Also build a temporary list of who the Neighbours are, to be used in GetAStarNeighbour().
int32 AStarMapPlanner::GetNumAStarNeighbours(AGOAPController* pAgent, AStarNodeAbstract* pAStarNode)
{
	// Sanity check.
	if (!pAStarNode) {
		return 0;
	...
	for (uint32 iEffect = 0; iEffect < kWSKey_Count; ++iEffect) {
		...
		// Determine if the effect matches any of the world state properties that are different in the current and goal world states.
		...
		if (pPropCur && pPropGoal) {
			if (pPropCur->pWSValue != pPropGoal->pWSValue) {
				...
				//ACTION_TYPE_LIST::TIterator* itAction = m_listEffectActions;
				//for (itAction = m_listEffectActions[eWSKey].GetData.cre  ; itAction != m_listEffectActions[eWSKey].Top(); ++itAction)
				//for (auto It = StrArr.CreateConstIterator(); It; ++It)
				
				for (auto itAction = m_listEffectActions[eWSKey].CreateIterator(); itAction; ++itAction) {
					eAction = *itAction;
	...
	// Sort Actions by precedence. This sorting will produce plans that have actions sorted by precedence,
...
ENUM_ASTAR_NODE_ID AStarMapPlanner::GetAStarNeighbour(AGOAPController* pAgent, AStarNodeAbstract* pAStarNode, int32 iNeighbour, AStarStorageAbstract* pAStarStorage)
{
	return ConvertID_AIAction2AStarNode(m_aNeighbourActions[iNeighbour]);
}

//Set AStar flags for a specified node.
void AStarMapPlanner::SetAStarFlags(ENUM_ASTAR_NODE_ID eAStarNode, uint64 dwFlags, uint64 dwMask)
{
...
uint64 AStarMapPlanner::GetAStarFlags(ENUM_ASTAR_NODE_ID eAStarNode)
{
