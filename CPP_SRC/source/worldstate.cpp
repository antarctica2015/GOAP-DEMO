...
void WorldState::SetWSProp(ENUM_WORLDSTATE_PROP_KEY ePropKey, ENUM_WORLDSTATE_PROP_TYPE eType, bool bValue)
{
	...
	//m_aWSProps[ePropKey].eVariableWSKey = ePropKey;

	//insure all bits (32) are cleared then set bool value

	m_aWSProps[ePropKey].iWSValue = 0;
	//m_aWSProps[ePropKey].fWSValue = 0;
	//m_aWSProps[ePropKey].pWSValue = NULL;
...
//set world state property
//override for ints
void WorldState::SetWSProp(ENUM_WORLDSTATE_PROP_KEY ePropKey, ENUM_WORLDSTATE_PROP_TYPE eType, int iValue)
{
	m_aWSProps[ePropKey].eWSKey  = ePropKey;
	m_aWSProps[ePropKey].eWSType = eType;
...
STRUCT_WORLDSTATE_PROP*	WorldState::GetWSProp(unsigned int iProp)
{
...
STRUCT_WORLDSTATE_PROP* WorldState::DereferenceWSProp( ENUM_WORLDSTATE_PROP_KEY ePropKey )
{
	...
	while( (pEvaluatedProp && kWSType_Variable ) == pEvaluatedProp->eWSType )
	{
		...
		{
			//log Error to file
			std::cout<< "WorldState::DereferenceWSProp: Recursive properties detected!"<< std::endl;
			pEvaluatedProp = NULL;
			break;
		}
...
void WorldState::CopyWorldState(WorldState& wsWorldState)
{
	...
	for(int iProp = 0; iProp < kWSKey_Count; ++iProp)
	{
...
unsigned int WorldState::GetNumWorldStateDifferences(WorldState &wsOtherWorldState)
{
	unsigned int iDifferences = 0;
...
unsigned int WorldState::GetNumUnsatisfiedWorldStateProps(WorldState& wsOtherWorldState)
{
	unsigned int iUnsatisfied = 0;
